...
- 能源报告与数据库：能源模式提供约 200 份近三年重点报告，并纳入 36 个经表格整理的国际能源数据库，覆盖化石能源、电池、新型电力系统、核能、聚变和碳捕集利用与封存。报告可按类型、国家或组织、年份和关键词检索；数据库条目展示维护机构、核心数据、主要用途、覆盖范围、更新情况及正式入口。每日任务继续增量发现新报告，数据库目录则按来源复核更新。
...
8. 公开首页和 PDF 只采用最新北京时间自然日，目标展示约 10 条，`daily_backfilled` 固定为 0。地图“本周”仍取最近 7 日窗口；位置识别会优先从标题、摘要和国家标签中抽取一个主地点，无法合理归类时才保留为“全球”。
