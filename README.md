...
- 置顶的全球气候现场可切换“今日队列 / 本周”；红点仅标记文本明确涉及的国家或地区，并通过自动错位避免密集点遮挡。
- 今日重要气候情报：只展示最新北京时间自然日的记录，不用往日内容补位。系统以多个短查询补充全球政策、极端天气、清洁能源和区域新闻，并在北京时间晚间再次刷新，目标约 10 条。同标题转载副本合并为一条公开记录，同时保留备用链接和来源观察，历史事实与追溯信息不会丢失。每条均包含中文标题、完整中文概要、议题、来源、发布时间和原文链接。
- 中国碳市场专题：以生态环境部、中国政府网和国家标准平台等一手材料为政策基线，分全国碳排放权交易市场、自愿减排市场、《巴黎协定》第六条、监测报告核查和产品碳足迹五类展示；每日任务同步检索新增记录并自动并入专题。
- 中国碳市场机构与全国名录：集中链接国家能源局、生态环境部、国家气候战略中心、应对气候变化司、认证和交易登记机构；检索表现有 3497 条跨年度公开单位记录，覆盖 31 个省级地区和全国碳市场四个纳入行业，可按单位、省份及行业检索，不展示北京地方试点名单。2025 年官方总量作为独立核对基线，避免把跨年合并记录误写成当年完整名录。
- 品牌与项目介绍：气候和能源模式分别使用格润气候治理、格润能源技术字标；“项目介绍”按项目介绍材料展示建设背景、发布历程和项目愿景。
- 新加坡专题：集中展示国家气候变化秘书处、总理公署、永续发展与环境部、国家环境局、贸工部和能源市场管理局，并按机构检索近一年气候政策、碳市场与能源转型动态；每条均返回官方原文。
- 中英文界面：右上角“EN”切换主要导航、专题、标题与摘要。英文新闻使用来源原始题名和摘要；经人工校编的官方政策条目同时提供中英文内容，不以机器生成文字替代证据。
...
- 企业情报：仅在“能源技术”模式显示。企业目录现有 200 余家可追溯且可中文展示的记录：基础层要求具备国家、行业、企业官网和至少两个中文字符的名称，来源为 Wikidata 结构化字段；无可靠中文名、短代号或繁体展示名不进入公开目录。深度层对重点企业补充经财报或官网核验的营收、利润、核心技术、项目和发展方向。站内新闻另识别“能源巨头项目、能源初创企业、企业合作”三类动态；企业地图可切换今日队列和本周。目录不声称穷尽全球全部企业，也不估算未披露财务数据。
- 企业深度档案：重点企业增加业务概况、财年与币种明确的营收和利润、核心技术、重点项目、发展方向及官方财报入口。上市公司使用年度报告口径；未公开可比财务数据的初创企业明确标注“未公开”，不作估算。
- 能源报告与数据库：能源模式提供约 200 份近三年重点报告，并纳入 36 个经表格整理的国际能源数据库，覆盖化石能源、电池、新型电力系统、核能、聚变和碳捕集利用与封存。报告可按类型、国家或组织、国际组织、年份和关键词检索；每日发现采用发布机构配额、详情页题名校验和中文简介质量门禁，导航文字或缺少内容依据的条目不会发布。
- 分类体系：气候与能源记录统一补充 ISO 3166-1 二字母、三字母国别代码，并按“联合国与气候机制、全球经济与政策协调、区域合作机制、国际能源机构”四组标注 G20、COP、UNFCCC、APEC、G7、ASEAN、IEA、IRENA、SCO、BRICS、EU、AU、CPTPP、OECD 与“一带一路”；COP 届次从文本动态提取，不写死年份。
- 移动端：顶部“菜单”提供全球现场、今日情报、语料分析、情报问答、文本数据库、团队信息、订阅和模式切换；订阅弹窗采用单列触屏布局，可直接提交邮箱或退订。
...
- 周报订阅：每周自动汇总最近 7 天每日简报，提炼高频议题、重点地区和推荐阅读；网页直接调用 Cloudflare Worker 订阅端点，加载其他数据失败时表单仍可使用，失败原因在弹窗内显示，不再自动唤起本地邮件应用。
...
- 专题页包括京津冀绿色转型、新能源、储能技术和人工智能数据中心。系统回看三年来源可追溯语料，采用专题范围门禁和专业二级分类；每日发布时复用已通过质量门禁的档案，不另行调用模型或新闻 API。
- 京津冀专题以政策库为统计口径，展示政策文件、已覆盖地区、发布机构和官方域名，并提供按地区、关键词、年份和题名检索的政策卡片；不再混用专题新闻证据量。
- 京津冀绿色转型政策库配置50个标准地域节点和65个官方政府域名。政策统一保存发布日期、发布机构、标题、省级地区、城市或区、行政区划代码、关键词及原文链接；GitHub Actions分批回填近三年政策并每周增量扫描，本地电脑无需持续运行。内部城市画像与短报告保存于 `analysis/`，不会复制到公开网站。
- 国际机制标签新增“一带一路”，与 ISO 3166-1 国别标签、国际组织标签和议题标签共同写入公开数据。
- 全国重点排放单位检索表整合公开的省级名录，并保留生态环境部公布的 2025 年全国总量和行业分布作为核对基线。名录按单位、地区和行业筛选，官方总入口只展示一次。
- 每个专题展示三年证据量、近30日与前30日变化、独立来源数、官方来源占比和二级分类分布，并可沿原文逐条核查。页面不再把新闻出现频次包装成国家能源转型绩效评分，也不展示缺少解释力的国家—议题连线图。
- 上述处理复用每日 06:30 与 18:30（北京时间）的既有任务，不增加定时运行次数。专题匹配和统计均在本地执行，因而不会额外消耗 Gemini 免费额度；外部来源仍按既有超时、重试和隔离规则运行。
...
5. `data/news_archive.json` 按 canonical URL 合并，保留内容哈希与首次/最近归档时间，按发布时间排序并裁剪为最多 100000 条；同一批合格记录还会增量合并到 `data/climate_text_corpus.jsonl`，使三年语料库继续随每日更新增长。
6. 质量门禁通过后才部署 Pages；失败时网站保持上一成功版本。
7. 合格档案和 `data/source_health.json` 由工作流机器人提交回仓库，使下一次定时运行能够增量合并并判断来源健康状态。
8. 公开首页和 PDF 只采用最新北京时间自然日，目标展示约 10 条，`daily_backfilled` 固定为 0。地图“本周”仍取最近 7 日窗口；位置识别会优先从标题、摘要和标准国别标签中抽取一个主地点，无法合理归类时才保留为“全球”。Google News 的补充检索覆盖俄罗斯、东欧、中亚、北非、日本、韩国及主要国际组织，但不以地域配额替代事实相关性门禁。
...
每周订阅发送从受管理令牌保护的 `CLIMATE_WEEKLY_SUBSCRIBERS_ENDPOINT` 读取 Cloudflare Workers KV 中的有效地址，也兼容 `CLIMATE_WEEKLY_SUBSCRIBERS` 静态 Secret；同时必须配置 `CLIMATE_SMTP_*`。网页只有在 Worker 明确确认写入后才提示订阅成功，邮箱不会写入静态站点或 Git 历史。每周一北京时间 08:00 的工作流读取最新周报快照并逐一发送；名单读取失败会明确报错，单个地址失败不会阻断后续地址，日志只显示人数和错误类型。

本仓库现提供 Cloudflare Workers KV 订阅端点实现，代码位于 `cloudflare/subscription-worker/`。完成一次性部署后，网页可直接订阅和退订，周报任务通过受管理令牌保护的接口读取有效收件人；详细步骤见 `docs/SUBSCRIPTION_SETUP.md`。订阅地址和管理令牌均不进入 Git 或公开网页。
...
```text
config/                 来源配置、关键词、专题资料、企业名录和人工校编覆盖
