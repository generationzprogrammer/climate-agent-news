...
- 京津冀专题增加区域目标追踪、三地政策工具检索、全国碳市场名录结构和三年语料证据趋势；统计由本地档案生成，不增加每日外部接口调用。
- 京津冀绿色转型政策库覆盖50个标准地域节点和65个官方政府域名。政策统一保存发布日期、发布机构、标题、省级地区、城市或区、行政区划代码、关键词及原文链接；GitHub Actions按10个批次回填近三年政策并每周增量扫描，本地电脑无需持续运行。
...
每周订阅发送可从受管理令牌保护的 `CLIMATE_WEEKLY_SUBSCRIBERS_ENDPOINT` 读取 Cloudflare Workers KV 中的有效地址，也兼容 `CLIMATE_WEEKLY_SUBSCRIBERS` 静态 Secret；同时必须配置 `CLIMATE_SMTP_*`。网页表单只向 `CLIMATE_SUBSCRIBE_ENDPOINT` 或 `CLIMATE_UNSUBSCRIBE_ENDPOINT` 提交，邮箱不会写入静态站点或 Git 历史。每周一北京时间 08:00 的工作流读取最新周报快照并逐一发送，日志只显示收件人数。
