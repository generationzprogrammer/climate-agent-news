...
| Grounded Q&A Agent | 否（公开静态站） | 规划时间/地区/议题，执行字段加权检索、样本比较、时间线和连续追问 | 证据不足时拒绝补写；所有结果附原文 |
...
10. **站内问答**：公开网页只读取已通过发布门禁的滚动档案；检索计划和比较结论在浏览器内生成，不向第三方发送用户问题或档案内容。

### 3.1 地域补录与来源均衡

- 来源均衡是质量约束后的二级目标，不以洲际配额替代新闻价值判断。
